  ...
  return (
    ...
    >
      ...
      {currentPlanId === 'trial' && (
        <div className={`rounded-2xl border p-4 flex items-center justify-between gap-3 ${
          ...
          <div className="flex items-center gap-3">
            <Clock className={`w-5 h-5 flex-shrink-0 ${trialDaysLeft <= 3 ? 'text-red-400' : trialDaysLeft <= 7 ? 'text-amber-400' : 'text-green-600'}`} />
            <div>
          ...
          <button
            ...
            className="btn-primary px-4 py-2 rounded-xl text-xs font-bold text-gray-900 whitespace-nowrap flex items-center gap-1.5"
          >
      ...
      {currentPlanId === 'trial' && !loading && (() => {
        ...
        return (
          <div className="bg-gray-50 rounded-3xl border border-green-500/20 p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-lg font-bold text-gray-900">เริ่มต้นใช้งาน</h2>
                <p className="text-xs text-gray-400 mt-0.5">ทำให้ครบเพื่อเริ่มรับลูกค้าอัตโนมัติ</p>
              </div>
      ...
      {!loading && totalReplies === 0 && (
        <div className="flex items-start gap-3 px-4 py-3.5 rounded-2xl bg-green-500/5 border border-green-500/10">
          ...
          <div>
            <p className="text-xs font-bold text-green-600 mb-0.5">MeowCat แนะนำ 🐾</p>
            <p className="text-xs text-gray-500 leading-relaxed">
              เพิ่มสินค้า/บริการใน{' '}
              <button onClick={() => navigate('/catalog')} className="text-green-600 hover:text-green-500 font-semibold transition-colors">รายการสินค้า</button>
      ...
      {orders.length > 0 && (
        ...
        >
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div>
              <p className="text-xs text-gray-400 font-semibold">ออเดอร์</p>
          ...
          <div className="flex gap-6 flex-wrap">
            <div>
              ...
              <p className={`text-lg font-extrabold ${pendingOrders.length > 0 ? 'text-amber-400' : 'text-gray-500'}`}>
                {loading ? '...' : pendingOrders.length}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-400 mb-0.5">รายได้วันนี้</p>
        ...
        <div className="lg:col-span-1 bg-gray-50 rounded-3xl border border-white/[0.06] p-4 sm:p-6 flex flex-col gap-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-0.5">รีเซ็ตวันที่ {usage?.resetDate ?? '1 เมษายน 2026'}</p>
            </div>
            <span className="px-3 py-1 text-xs font-bold rounded-full bg-green-500/15 text-green-600 border border-green-500/20">
          ...
          <div>
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-gray-400 text-sm">/ {(usage?.limit ?? 2000).toLocaleString()} ข้อความ</span>
            </div>
            ...
            <p className="text-xs text-gray-400 mt-2">ใช้ไปแล้ว {usagePercent}% ของโควต้าเดือนนี้</p>
          </div>
          ...
          <button
            onClick={() => navigate('/subscription')}
            className="btn-primary px-5 py-3 rounded-xl text-sm font-bold text-gray-900 flex items-center justify-center gap-2 hover:shadow-xl hover:shadow-green-500/20 transition-all"
          >
            Upgrade แผน
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-gray-50 rounded-3xl border border-white/[0.06] p-6 flex flex-col gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400 text-xs mt-1">บทสนทนาใน 24 ชั่วโมงที่ผ่านมา</p>
            </div>
      ...
      <div className="bg-gradient-to-br from-green-500/10 to-green-400/5 rounded-3xl border border-green-500/20 p-6">
        ...
        {!loading && totalReplies === 0 ? (
          <div className="flex flex-col items-center gap-4 py-2 text-center">
            ...
            <button
              onClick={() => navigate('/bot')}
              className="mt-1 px-5 py-2.5 rounded-xl bg-green-500 hover:bg-green-400 text-gray-900 text-sm font-bold transition-colors shadow-lg shadow-green-500/20"
            >
      ...
      <div className="bg-gray-50 rounded-3xl border border-white/[0.06] p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <button
            onClick={() => navigate('/bot')}
            className="text-sm text-green-600 hover:text-green-500 font-semibold flex items-center gap-1 transition-colors"
          >
        ...
        {bot ? (
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-green-500/20 to-green-500/5 border border-green-500/20 flex items-center justify-center">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1">
                ...
                <span className={`flex items-center gap-1.5 px-3 py-1 text-xs font-bold rounded-full ${
                  bot.status === 'online'
                    ...
                    : 'bg-zinc-500/15 text-gray-500 border border-zinc-500/20'
                }`}>
              ...
              <p className="text-gray-500 text-sm">{bot.businessName} · {bot.channelId}</p>
            </div>
          </div>
        ) : (
          <p className="text-gray-400 text-sm">ยังไม่มีบอทที่เชื่อมต่อ — ไปที่ <strong className="text-green-600">ตั้งค่าบอท</strong> เพื่อเริ่มต้น</p>
        )}
      </div>
    ...
    <div className={`bg-gray-50 rounded-3xl border border-white/[0.06] p-5 hover:border-green-500/20 transition-all animate-fade-in ${delay}`}>
      ...
      {offlineAction && (
        <button
          ...
          className="mt-3 flex items-center gap-1.5 text-xs text-green-600 hover:text-green-500 font-semibold transition-colors"
        >
          ตั้งค่าเดี๋ยวนี้ →
        </button>
      )}
      ...
      <div className="flex items-center gap-4">
        <div className="relative w-24 h-24 flex-shrink-0">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-lg font-extrabold text-gray-900">{value === null ? '...' : `${pct}%`}</span>
          </div>
        </div>
