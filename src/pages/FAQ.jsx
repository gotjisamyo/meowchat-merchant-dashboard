...
export default function FAQ({ setSidebarOpen }) {
  ...
  return (
    <PageLayout
      ...
      actions={
        <button onClick={load} className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-black/[0.08] text-xs text-gray-500 hover:text-gray-900 hover:border-green-500/30 transition-colors">
    ...
    >
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          {/* Question list */}
          {questions.map((q) => (
            <div key={q.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-start gap-4">
              <div className="w-9 h-9 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center flex-shrink-0">
                <MessageSquare className="w-4 h-4 text-amber-400" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-gray-900 font-medium leading-relaxed">{q.question}</p>
                <p className="text-xs text-gray-400 mt-1">ถามมาแล้ว {q.count} ครั้ง · ล่าสุด {q.date && !isNaN(new Date(q.date)) ? new Date(q.date).toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}</p>
              </div>
              {addedIds.has(q.id) ? (
